...
function Reprobados() {
  const [calificacionesReprobadas, setCalificacionesReprobadas] = useState([]);
  ...
  useEffect(() => {
    const fetchCalificacionesReprobadas = async () => {
      try {
        const response = await axios.get("http://localhost:3000/calificaciones/reprobadas");
        setCalificacionesReprobadas(response.data);
      } catch (error) {
        console.error("Error al obtener calificaciones reprobadas:", error);
      }
    };

    fetchCalificacionesReprobadas();
  }, []);

  const handleEditClick = (index) => {
    const selectedCalificacion = calificacionesReprobadas[index];
  ...
  try {
    ...
    console.error("Error al actualizar calificación reprobada:", error);
  }
};
  ...
  return (
    <div>
      ...
      <nav className="bg-white border-gray-200 dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <a href="https://flowbite.com/" className="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Tabla de Calificaciones Reprobadas</span>
          </a>
          ...
          <div className="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              <li>
                <a href="calificaciones" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Calificaciones</a>
              </li>
              <li>
                <a href="reprobados" className="block py-2 px-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white md:dark:text-blue-500" aria-current="page">Reprobados</a>
              </li>
              <li>
     ...
     {/* Tabla de calificaciones reprobadas */}
<section className="flex flex-col justify-center items-center h-screen bg-gray-50 dark:bg-gray-900">
  <div className="relative overflow-x-auto shadow-md sm:rounded-lg bg-blue-900 w-full sm:w-auto mb-8">
    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        {calificacionesReprobadas.map((calificacion, index) => (
